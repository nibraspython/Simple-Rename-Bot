...
def download_progress(d):
    if d['status'] == 'downloading':
        ...
        progress_str = f"Downloading: {progress:.2f}% - {humanbytes(downloaded_bytes)} of {humanbytes(total_bytes)}"
        speed_str = f"Speed: {humanbytes(speed)}/s" if speed else ""
        eta_str = f"ETA: {time.strftime('%H:%M:%S', time.gmtime(eta))}" if eta else ""

        # Update the message in Telegram
        message_text = f"{progress_str}\n{speed_str}\n{eta_str}"
        if 'msg' in d:
            try:
                d['msg'].edit_text(message_text)
