...
@Client.on_message(filters.private & filters.command("video") & filters.user(ADMIN))
...
async def download_and_upload_video(bot, msg):
    ...
    sts = await msg.reply_text("🔄 Downloading the video from Dailymotion.....📥")
    ...
    if not video_files:
        return await sts.edit("Failed to download the video.")
